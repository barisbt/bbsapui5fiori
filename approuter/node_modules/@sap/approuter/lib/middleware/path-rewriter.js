...
module.exports = function(req, matchHttpMethods /* default undefined, does not make sense for WebSockets */) {
  ...
  let dynamicIDP = urlUtils.getQueryParam(req, sapIdpParam, true);
  ...
  for (let i = 0; i < routes.length; i++) {
    ...
    if (!matchHttpMethods || httpMethodsMatch(route.httpMethods, req.method)) {
      if (dynamicIDP && route.dynamicIdentityProvider) {
        route.identityProvider = dynamicIDP;
      }
      let rewrittenUrl = applyRewriteRule(req, route, routerConfig);
      return rewrittenUrl;
    }
